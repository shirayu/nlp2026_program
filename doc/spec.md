...
- `presentations[*].start_time` / `end_time` は、UI設定「時間情報があれば発表単位で表示」が `ON` かつ時点指定ありの場合に時間フィルタで利用する（デフォルト `OFF`）
- 上記設定が `OFF` の場合は従来どおりセッション単位で時点判定する
- 上記設定が `ON` でも、時間情報を持たない発表を1件でも含むセッションはセッション全体の時間枠で扱う
